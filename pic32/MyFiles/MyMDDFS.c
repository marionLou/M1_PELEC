...
void MyMDDFS_loadOneshow(int Level) {

    char base_name[15] = "Level_";
    char img_name[21];
    ...
    if (Level==1 || Level==2 || Level==3){
        sprintf(img_name, "Level_%d.bmp", Level);
        if (MyMDDFS_ReadImg(img_name)) {
            MyConsole_SendMsg("An error occurred while opening a file.\n\t");
            MyConsole_SendMsg("Verify the SD card and your inputs, then try again!\n\t");
        }
    }
    else MyConsole_SendMsg("Wrong level given, try again");
